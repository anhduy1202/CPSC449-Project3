...
def get_available_classes(student_id: str):
    ...
    if not student_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No student found"
        )
...
def enroll_student_in_class(student_id: str, class_id: str):
    ...
    if not class_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No class found"
        )
    ...
    if class_data["Frozen"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Enrollment is frozen"
        )
    ...
    if class_data["currentEnroll"] >= class_data["maxEnroll"]:
        ...
        if not r.exists(waitlist_key):
            ...
            update_last_modified(class_id, r)
            ...
            if r.llen(waitlist_key) < MAX_WAITLIST:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Unable to add student to waitlist due to already having max number of waitlists",
                )
    ...
    update_finished = qh.update_current_enroll(
        dynamodb_client, class_id, new_enrollment
    )
    ...
    if not class_data["Frozen"]:
        if r.exists(f"waitlist:{class_id}"):
            ...
            return {
                ...
                "Class": updated_class_data["Detail"],
            }
...
@router.get(
    "/students/{student_id}/waitlist/{class_id}",
    tags=["Waitlist"],
    summary="Get waitlist position for a student in a class",
)
def view_waiting_list(student_id: str, class_id: str, response: Response):
    response.headers["Last-Modified"] = get_last_modified(class_id, r)
    # check if student exists in the database
    student_data = qh.query_student(dynamodb_client, student_id)
    if not student_data:
        raise HTTPException(
    ...
    if not r.exists(waitlist_key):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No waitlist found"
        )
    ...
    if id not in waitlist_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Student is not on waitlist"
        )
...
@router.delete(
    "/students/{student_id}/waitlist/{class_id}",
    tags=["Waitlist"],
    summary="Remove a student from a waiting list",
)
...
@router.get(
    "/instructors/{instructor_id}/classes/{class_id}/enrollment",
    tags=["Instructor"],
    summary="Get current enrollment for class",
)
def get_instructor_enrollment(instructor_id: str, class_id: str):
    ...
    if not instructor_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No instructor found"
        )
    # check if class exists in the database
    class_data = qh.query_class(dynamodb_client, class_id)
    if not class_data:
    ...
    class_instructor = qh.query_class_instructor(
        dynamodb_client, instructor_id, class_id
    )
    ...
    if not enrollment_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No enrollment found"
        )
...
@router.get(
    ...
    tags=["Instructor"],
    summary="Get students who dropped the class",
)
def get_instructor_dropped(instructor_id: str, class_id: str):
    ...
    if not dropped_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No dropped found"
        )
...
@router.post(
    ...
    tags=["Instructor"],
    summary="Instructor administratively drop students",
)
...
def freeze_automatic_enrollment(class_id: str):
    ...
    if class_data["Frozen"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Class is already frozen"
        )
    # Freeze the class
