...
def query_student(dynamodb_client, student_id):
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"s#{student_id}"}, "SK": {"S": f"s#{student_id}"}},
    }
    try:
    ...
    input = {
        "RequestItems": {
            "TitanOnlineEnrollment": {
                "Keys": [
                    {"PK": {"S": f"{student_id}"}, "SK": {"S": f"{student_id}"}}
...
def check_class_exists(dynamodb_client, class_id):
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"c#{class_id}"}},
    }
    try:
        response = dynamodb_client.get_item(**input)
        # Parse data from response
...
def update_enrolled_class(dynamodb_client, student_id, class_id):
    ...
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"s#{student_id}"}},
    }
    try:
    ...
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"s#dropped#{student_id}"}},
    }
    try:
        response = dynamodb_client.delete_item(**input)
    except ClientError as error:
...
def query_instructor(dynamodb_client, instructor_id):
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"i#{instructor_id}"}, "SK": {"S": f"i#{instructor_id}"}},
    }
    try:
        response = dynamodb_client.get_item(**input)
        # Parse data from response
...
def query_class_instructor(dynamodb_client, instructor_id, class_id):
    input = {
        ...
        "ExpressionAttributeValues": {
            ":77100": {"S": f"i#{instructor_id}"},
            ":77101": {"S": f"c#{class_id}"},
        },
    }
    try:
        response = dynamodb_client.query(**input)
...
def drop_student_from_class(dynamodb_client, student_id, class_id):
    ...
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"s#enrolled#{student_id}"}},
    }
    try:
        response = dynamodb_client.delete_item(**input)
    except ClientError as error:
...
def create_class(dynamodb_client, class_detail):
    ...
    filtered_class_detail = {
        ...
        for k in serialized_class_detail
        if k not in ["maxEnroll", "InstructorId"]
    }
    ...
    input = {
        ...
        "ExpressionAttributeNames": {"#cd420": "PK", "#cd421": "SK"},
        "ExpressionAttributeValues": {
            ":cd420": {"S": f"c#{available_class_id}"},
            ":cd421": {"S": "s#"},
        },
    }
    student_ids = []
...
def delete_class(dynamodb_client, class_id):
    ...
    input = {
        "TableName": "TitanOnlineEnrollment",
        "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"i#{class_id}"}},
    }
    try:
        response = dynamodb_client.delete_item(**input)
    ...
    for student_id in student_ids:
        input = {
            "TableName": "TitanOnlineEnrollment",
            "Key": {"PK": {"S": f"c#{class_id}"}, "SK": {"S": f"{student_id}"}},
        }
        try:
            response = dynamodb_client.delete_item(**input)
        except ClientError as error:
