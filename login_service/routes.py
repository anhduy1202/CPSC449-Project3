...
database_reps = itertools.cycle(
    ["./var/secondary/fuse/database.db", "./var/tertiary/fuse/database.db"]
)
...
def register_user(user_data: Users, db: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        db.execute(
            ...
            """,
            (user_data.uid, user_data.name, user_data.password, roles_str),
        )
        db.commit()
        return user_data
...
@router.post("/login")
def verify_user(
    login_data: Userlogin, db: sqlite3.Connection = Depends(get_db_replicas)
):
    cursor = db.cursor()
    ...
    if flag:
        return utils.generate_claims(
            login_data.username, user_data["uid"], user_data["roles"].split(",")
        )
    else:
